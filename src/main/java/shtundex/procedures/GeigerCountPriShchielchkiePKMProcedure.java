...
public class GeigerCountPriShchielchkiePKMProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		} else if ((entity.getCapability(ShtundexModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ShtundexModVariables.PlayerVariables())).Radiation < 100) {
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal("Your RAD is normal. <100"), false);
		} else if ((entity.getCapability(ShtundexModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ShtundexModVariables.PlayerVariables())).Radiation > 700) {
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal("WARNING! Your RAD too high. >700"), false);
		} else if ((entity.getCapability(ShtundexModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ShtundexModVariables.PlayerVariables())).Radiation > 300) {
			if (!world.isClientSide() && world.getServer() != null)
