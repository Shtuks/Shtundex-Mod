...
package shtundex.command;

import shtundex.procedures.JJCActiveProcedure;
...
public class ShtuxianpriorityCommand {
	@SubscribeEvent
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("shtundexcompatibility")
				...
				.then(Commands.argument("logic", BoolArgumentType.bool()).then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(arguments -> {
					...
					JJCActiveProcedure.execute(world, arguments);
					return 0;
				})).executes(arguments -> {
					Level world = arguments.getSource().getUnsidedLevel();
					double x = arguments.getSource().getPosition().x();
