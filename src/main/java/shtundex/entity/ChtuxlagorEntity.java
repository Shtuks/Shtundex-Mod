...
public class ChtuxlagorEntity extends PathfinderMob {
	public ChtuxlagorEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ShtundexModEntities.CHTUXLAGOR.get(), world);
	}

	public ChtuxlagorEntity(EntityType<ChtuxlagorEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = ChtuxlagorEntity.this.getRandom();
				double dir_x = ChtuxlagorEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = ChtuxlagorEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = ChtuxlagorEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
